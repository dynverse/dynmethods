...
readr::read_lines(glue::glue("inst/doc/{file}.md")) %>% stringr::str_replace_all(paste0(rprojroot::find_rstudio_root_file(), "/"), "") %>% readr::write_lines(glue::glue("inst/doc/{file}.md"))
```
...
The wrapper can be accessed using the `description_aga` function.
```{r}
