...
In some cases, we already provide functions which do the calculation of milestone_percentages or progressions. For example, when a method returns only a pseudotimes:
...
```{r}
dynplot::plot_dimred(trajectory, "pseudotimes", expression_source = expression)
```
...
```{r}
library(dynplot)
dynplot::plot_dimred(model, color_cells = "pseudotimes" , expression_source = task$expression)
```
