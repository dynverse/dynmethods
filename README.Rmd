...
df <- methods %>% 
  ...
  mutate(
    ...
    `Docker container` = ifelse(
      containerised, 
      glue::glue("[✓ ![Docker hub](https://img.shields.io/docker/pulls/{docker_container}.svg)]({dockerhub_location})"), 
      ""
    )
  )
knitr::kable(df %>% select(Name = method_name, Wrapper, `R wrapper`, `Docker container`))
