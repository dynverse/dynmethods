...
data("methods")
methods <- methods %>% filter(!source %in% c("control")) # controls are not actual TI methods
```
...
news_normal <- news_md %>% 
  str_replace_all("^# dynmethods", "dynmethods") %>% 
  str_replace_all("\\[[^\\]]*\\]\\(([^\\)]*)\\)", "\\1")
readr::write_lines(news_normal, "inst/NEWS")

# creating text for readme
ix <- which(str_detect(news_md, "^# dynmethods"))
