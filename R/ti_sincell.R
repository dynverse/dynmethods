...
#' 
#' @param distance_method Parameter; Distance method to be used. The available
...
#' parameter 'bins'., Domain: {euclidean, cosine, pearson, spearman, L1, MI},
#' Default: euclidean, Format: character.
#' @param dimred_method Parameter; Dimensionality reduction algorithm to be used.
...
#' @param clust.method Parameter; If clust.method="max.distance", clusters are
...
#' @param mutual Parameter; If clust.method="knn" and "mutual=TRUE", Unsupervised
#' K-Mutual Nearest Neighbours (K-MNN) clustering is performed, that is: only
#' reciprocal k nearest neighbors are connected., Default: TRUE, Format: logical.
#' @param max.distance Parameter; In max.distance algorithm, select up to which
#' distance the points will be linked., Domain: U(0, 5), Default: 0, Format:
#' numeric.
#' @param k Parameter; If clust.method="knn", k is an integer specifying the
#' number of nearest neighbors to consider in K-NN and K-KNN., Domain: U(1, 99),
#' Default: 3, Format: integer.
#' @param shortest.rank.percent Parameter; In percent algorithm, select the
#' percent of shortest distances will be represented as links., Domain: U(0, 100),
#' Default: 10, Format: numeric.
#' @param graph.algorithm Parameter; Graph building algorithm to be used: the
...
#' character.
#' @param graph.using.cells.clustering Parameter; If
...
#' by function sc_clusterObj()., Default: FALSE, Format: logical.
#' @param k_imc Parameter; If IMC algorithm is selected, the number of nearest
#' neighbors used in the underlying K-Mutual Nearest Neighbour (K-MNN) algorithm
#' is set to k., Domain: U(1, 99), Default: 3, Format: integer.
#' @param pct_leaf_node_cutoff Parameter; Leaf nodes are iteratively removed until
...
#' milestones being created., Domain: U(0.01, 0.8), Default: 0.5, Format: numeric.
#' 
