...
#' of th training set multiplied by \code{nbfactor}. Default value is 5. Domain:
#' U(2, 100). Default: 5. Format: integer.
...
#' the \code{minnr} cells with the highest similarity to a cell in the training
#' set are selected for classification. If \code{z} is not \code{NULL} it is used
#' as the similarity matrix for this step. Otherwise, \code{1-cor(x)} is used.
#' Default value is 5. Domain: U(2, 100). Default: 5. Format: integer.
...
#' the principal curve. If \code{NULL} then only cells with a significant bias >1
#' are included for each trajectory. The bias is computed as the ratio of the
#' number of votes for a trajectory and the number of votes for the trajectory
#' with the second largest number of votes. By this means only the trajectory with
#' the largest number of votes will receive a bias >1. The siginifcance is
#' computed based on counting statistics on the difference in the number of votes.
#' A significant bias requires a p-value < 0.05. Domain: U(0, 1). Default: 0.4.
#' Format: numeric.
