...
run_fun <- function (expression, distance_method = "euclidean", dimred_method = "none", 
    clust.method = "max.distance", mutual = TRUE, max.distance = 0, 
    k = 3, shortest.rank.percent = 10, graph.algorithm = "MST", 
    graph.using.cells.clustering = FALSE, k_imc = 3, pct_leaf_node_cutoff = 0.5) 
{
    ...
    SO <- SO %>% sincell::sc_distanceObj(method = distance_method)
    if (dimred_method != "none") {
        SO <- SO %>% sincell::sc_DimensionalityReductionObj(method = dimred_method)
    }
    SO <- SO %>% sincell::sc_clusterObj(clust.method = clust.method, 
        mutual = mutual, max.distance = max.distance, shortest.rank.percent = shortest.rank.percent, 
        k = k)
    SO <- SO %>% sincell::sc_GraphBuilderObj(graph.algorithm = graph.algorithm, 
        graph.using.cells.clustering = graph.using.cells.clustering, 
        k = k_imc)
    tl <- tl %>% add_timing_checkpoint("method_aftermethod")
    cell_graph <- SO$cellstateHierarchy %>% igraph::as_data_frame() %>% 
        rename(length = weight) %>% mutate(directed = FALSE)
    gr <- SO$cellstateHierarchy
    deg <- igraph::degree(gr)
    prev_deg <- deg * 0
    while (length(deg) > 10 && mean(deg <= 1) > pct_leaf_node_cutoff && 
        any(deg != prev_deg)) {
    ...
    to_keep <- setNames(rownames(expression) %in% names(igraph::V(gr)), 
        rownames(expression))
