...
run_fun <- function (expression, start_id, features_id = NULL, end_id = NULL, 
    kmin = 10, m = 2, verbose = FALSE) 
{
    ...
    cell_graph <- igraph::as_data_frame(subgr, "edges") %>% dplyr::select(from, 
        to, length = weight) %>% mutate(from = rownames(expr_filt)[from], 
        to = rownames(expr_filt)[to], directed = TRUE)
    sh_p_to_ends <- igraph::shortest_paths(subgr, start, ends)
    nodes_to_keep <- unique(sh_p_to_ends$vpath %>% unlist)
    to_keep <- setNames(igraph::V(traj_graph) %in% nodes_to_keep, 
        rownames(expr_filt))
    wrap_prediction_model(cell_ids = rownames(expr_filt)) %>% 
        add_cell_graph(cell_graph = cell_graph, to_keep = to_keep, 
            traj_graph = subgr, start = start, ends = ends, is_kept = to_keep) %>% 
        add_dimred(dimred = traj_lle) %>% add_timings(timings = tl %>% 
